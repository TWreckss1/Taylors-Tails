...
function detailsTable(b: BookingEmailDetails): string {
  const rows = [
    ["Dog", b.dogName],
    ["Service", b.service],
...
export async function sendNewBookingEmails(booking: BookingEmailDetails, siteOrigin: string) {
  ...
  if (ownerEmail) {
    const ownerHtml = emailShell(
      ...
      <p style="font-size:14px;">
        <a href="${siteOrigin}/admin/bookings" style="color:#8B9E7A;font-weight:bold;">Open the admin panel</a> to confirm or decline.
      </p>`
    );
    await sendEmail(
      ownerEmail,
      "Taylor",
      `New booking: ${booking.dogName} — ${booking.date} ${booking.time}`,
      ownerHtml
    );
  }
}

export async function sendBookingConfirmedEmail(booking: BookingEmailDetails, siteOrigin: string) {
  const hasDeposit = Boolean(booking.depositAmount && booking.depositAmount > 0 && booking.id);
  ...
  const html = emailShell(
    ...
    ${
      hasDeposit
        ? `<p style="color:#7A7265;font-size:14px;line-height:1.6;">
            A deposit of <strong style="color:#2C2A25;">£${booking.depositAmount!.toFixed(2)}</strong> is required to secure this booking.
          </p>
          ${ctaButton(`${siteOrigin}/pay/${booking.id}`, `Pay Your £${booking.depositAmount!.toFixed(2)} Deposit`)}`
        : ""
    }
    <p style="color:#7A7265;font-size:13px;line-height:1.6;">
    ...
    <p style="color:#7A7265;font-size:13px;line-height:1.6;">
      ...
      <a href="${siteOrigin}/review" style="color:#8B9E7A;font-weight:bold;">leave us a review</a>.
    </p>`
  );
  await sendEmail(
    booking.ownerEmail,
...
export async function sendBookingCancelledEmail(booking: BookingEmailDetails) {
  const html = emailShell(
...
export async function sendDepositReceiptEmail(booking: Booking) {
  if (!booking.depositAmount) return;
  const html = emailShell(
    `Deposit received — thank you!`,
    `<p style="color:#7A7265;font-size:14px;line-height:1.6;">
      We've received your deposit of <strong style="color:#2C2A25;">£${booking.depositAmount.toFixed(2)}</strong> for ${booking.dogName}'s appointment. You're all set!
    </p>
    ${detailsTable(booking)}
    <p style="color:#7A7265;font-size:13px;line-height:1.6;">
      See you soon! 🐾
  ...
  await sendEmail(
    booking.ownerEmail,
    booking.ownerName,
    `Deposit received — ${booking.dogName}'s appointment is all set`,
    html
  );
}
